...
export default async function usuarioRoute(app) {
    app.post("/registrar-plano-gratuito", async (request, reply) => {
        ...
        try {
            ...
            logger.error({ error: err.message, payload: { email: payload.email, plano: payload.plano_id } }, "Falha no Endpoint de Cadastro no Plano Gratuito.");
    ...
    app.post("/registrar-plano-essencial", async (request, reply) => {
        ...
        try {
            ...
            logger.error({ error: err.message, payload: { email: payload.email, plano: payload.plano_id } }, "Falha no Endpoint de Cadastro no Plano Essencial.");
            const status = err.message.includes("já está em uso") ? 409 : 400;
            return reply.status(status).send({ error: err.message });
        }
    });
    app.post("/registrar-plano-completo", async (request, reply) => {
        ...
        try {
            ...
            logger.error({ error: err.message, payload: { email: payload.email, plano: payload.plano_id } }, "Falha no Endpoint de Cadastro no Plano Completo.");
            const status = err.message.includes("já está em uso") ? 409 : 400;
            return reply.status(status).send({ error: err.message });
        }
    });
    app.delete("/cancelar-assinatura/:id", async (request, reply) => {
    ...
    app.patch("/desistir-cancelar-assinatura/:id", async (request, reply) => {
        const usuarioId = request.params.id;
        try {
    ...
    app.post("/upgrade-plano", async (request, reply) => {
        const authUid = request.user?.id;
        const { plano_id, usuario_id } = request.body;
        if (!plano_id) {
    ...
    app.post("/downgrade-plano", async (request, reply) => {
        const authUid = request.user?.id;
        const { plano_id, usuario_id } = request.body;
        if (!plano_id) {
            return reply.status(400).send({ error: "Plano é obrigatório." });
    ...
    app.post("/trocar-subplano", async (request, reply) => {
        const authUid = request.user?.id;
        const { subplano_id, usuario_id } = request.body;
        if (!subplano_id) {
    ...
    app.post("/criar-assinatura-completo-personalizado", async (request, reply) => {
        const authUid = request.user?.id;
        const { quantidade, usuario_id } = request.body;
    ...
    app.get("/:id/verificar-selecao-manual-necessaria", async (request, reply) => {
        ...
        try {
            ...
            const assinaturaPendente = cobrancaPendente.assinaturas_usuarios;
            if (!assinaturaPendente) {
                return reply.status(200).send({ precisaSelecaoManual: false });
            }
            const plano = assinaturaPendente.planos;
            ...
            logger.error({ error: err.message, usuarioId: usuarioIdFinal }, "Falha ao verificar se precisa seleção manual.");
            return reply.status(500).send({ error: err.message });
        }
    });
    app.get("/:id/passageiros-para-selecao", async (request, reply) => {
        const usuarioId = request.params.id;
        const { tipo, franquia } = request.query;
        if (!tipo || !franquia) {
            return reply.status(400).send({ error: "Tipo e franquia são obrigatórios." });
        }
        try {
            const passageiros = await passageiroService.listarPassageirosParaSelecao(usuarioId, tipo, Number(franquia));
            return reply.status(200).send(passageiros);
        }
        catch (err) {
            logger.error({ error: err.message, usuarioId, tipo, franquia }, "Falha ao listar passageiros para seleção.");
            return reply.status(400).send({ error: err.message });
        }
    });
    app.post("/:id/salvar-selecao-passageiros", async (request, reply) => {
        const usuarioId = request.params.id;
        const { cobrancaId, passageiroIds, tipo, franquia } = request.body;
        ...
        try {
            ...
            logger.error({ error: err.message, usuarioId, cobrancaId, passageiroIds }, "Falha ao salvar seleção de passageiros.");
            return reply.status(400).send({ error: err.message });
        }
    });
    app.post("/:id/selecionar-passageiros-cobranca-automatica", async (request, reply) => {
        const usuarioId = request.params.id;
        const { passageiroIds, franquia, tipoDowngrade, subplanoId, quantidadePersonalizada, tipo, planoId, precoAplicado, precoOrigem, cobrancaId } = request.body;
        ...
        try {
            ...
            if (tipoDowngrade) {
                const resultado = await confirmarDowngradeComSelecao(usuarioId, passageiroIds, franquia, tipoDowngrade, subplanoId, quantidadePersonalizada);
                ...
                if (cobrancaId && tipo === "upgrade") {
                    ...
                    if (precoAplicado !== undefined && precoAplicado > 0) {
                        try {
                            const pixData = await gerarPixAposSelecaoManual(usuarioId, tipo, precoAplicado, precoOrigem || "personalizado", planoId, subplanoId, quantidadePersonalizada, cobrancaId);
                            return reply.status(200).send({
                                success: true,
                                ...pixData,
                            });
                    ...
                    const resultado = await passageiroService.confirmarSelecaoPassageiros(usuarioId, passageiroIds, franquia);
                    return reply.status(200).send(resultado);
                }
            }
        }
        catch (err) {
            logger.error({ error: err.message, usuarioId, passageiroIds, franquia }, "Falha ao confirmar seleção de passageiros.");
            return reply.status(400).send({ error: err.message });
        }
    });
}
