...
export async function checkUserStatus(cpfcnpj, email, telefone) {
...
export async function criarUsuarioAuth(email, senha, usuario_id) {
    ...
    return {
        ...
        user: authData.user, // Type assertion necessário pois o tipo do Supabase pode variar
    };
}
export async function rollbackCadastro({ usuarioId, authUid, assinaturaId, cobrancaId, }) {
    try {
...
export async function iniciaRegistroPlanoGratuito(payload) {
    let usuarioId = null;
    let authUid = null;
...
export async function iniciaRegistroPlanoEssencial(payload) {
    let usuarioId = null;
    let authUid = null;
    let assinaturaId = null;
    try {
...
export async function iniciarRegistroPlanoCompleto(payload) {
    let usuarioId = null;
    let authUid = null;
    let assinaturaId = null;
    let cobrancaId = null;
    try {
        ...
        if (payload.quantidade_personalizada) {
            ...
            const { data: plano, error: planoError } = await supabaseAdmin
                .from("planos")
                .select("id, nome, preco, preco_promocional, promocao_ativa, franquia_cobrancas_mes")
                .eq("id", planoSelecionadoId)
                .single();
            if (planoError || !plano)
                throw new Error("Erro ao encontrar o plano selecionado.");
            precoAplicado = Number(plano.promocao_ativa ? plano.preco_promocional ?? plano.preco : plano.preco);
...
export async function cancelarAssinatura(payload) {
...
export async function desistirCancelarAssinatura(usuarioId) {
    if (!usuarioId) {
 ...
 */
async function getAssinaturaAtiva(usuarioId) {
 ...
 */
async function cancelarCobrancaPendente(usuarioId) {
 ...
 */
async function limparAssinaturasPendentes(usuarioId) {
    try {
 ...
 */
function isUpgrade(slugAtual, slugNovo) {
    const ordem = {
        [PLANO_GRATUITO]: 1,
        [PLANO_ESSENCIAL]: 2,
        [PLANO_COMPLETO]: 3,
 ...
 */
async function calcularPrecoPersonalizado(quantidade) {
    // Buscar o plano base Completo
    const { data: planoCompletoBase, error: planoBaseError } = await supabaseAdmin
        .from("planos")
        .select("id")
    ...
    const precoBase = Number(maiorSubplano.promocao_ativa
 ...
 */
function calcularPrecosEFranquia(plano) {
    const precoAplicado = Number(plano.promocao_ativa ? plano.preco_promocional ?? plano.preco : plano.preco);
    const precoOrigem = plano.promocao_ativa ? "promocional" : "normal";
 ...
 */
export async function upgradePlano(usuarioId, novoPlanoId) {
    try {
 ...
 * - Mantém vigência original
 */
export async function downgradePlano(usuarioId, novoPlanoId) {
    try {
        // Buscar assinatura ativa
        const assinaturaAtual = await getAssinaturaAtiva(usuarioId);
        const planoAtual = assinaturaAtual.planos;
 ...
 */
export async function trocarSubplano(usuarioId, novoSubplanoId) {
    try {
        // Buscar assinatura ativa
        const assinaturaAtual = await getAssinaturaAtiva(usuarioId);
        const planoAtual = assinaturaAtual.planos;
 ...
 * - Mantém vigência original se houver assinatura atual
 */
export async function criarAssinaturaCompletoPersonalizado(usuarioId, quantidade) {
    try {
 ...
 */
async function fazerDowngradeSubplanoInterno(usuarioId, subplanoId) {
    const assinaturaAtual = await getAssinaturaAtiva(usuarioId);
    // Buscar novo subplano
    const { data: novoSubplano, error: planoError } = await supabaseAdmin
        .from("planos")
 ...
 */
async function fazerDowngradePersonalizadoInterno(usuarioId, quantidade) {
    const { precoCalculado } = await calcularPrecoPersonalizado(quantidade);
    const assinaturaAtual = await getAssinaturaAtiva(usuarioId);
    // Cancelar cobranças pendentes
    await cancelarCobrancaPendente(usuarioId);
 ...
 */
export async function confirmarDowngradeComSelecao(usuarioId, passageiroIds, franquia, tipoDowngrade, subplanoId, quantidadePersonalizada) {
    try {
 ...
 */
export async function gerarPixAposSelecaoManual(usuarioId, tipo, precoAplicado, precoOrigem, planoId, subplanoId, quantidadePersonalizada, cobrancaId) {
    try {
