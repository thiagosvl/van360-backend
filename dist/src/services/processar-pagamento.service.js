...
import { ASSINATURA_COBRANCA_STATUS_PAGO, ASSINATURA_COBRANCA_STATUS_PENDENTE_PAGAMENTO, ASSINATURA_COBRANCA_STATUS_CANCELADA, ASSINATURA_COBRANCA_TIPO_PAGAMENTO_PIX, PLANO_COMPLETO, } from "../config/contants.js";
/**
 ...
 */
export async function processarPagamentoCobranca(cobranca, dadosPagamento, contextoLog = {}) {
    ...
    try {
        ...
        const { vigenciaFim, novoAnchorDate } = await calcularVigenciaFimEAnchorDate(cobranca, dadosPagamento.dataPagamento, logContext);
        ...
        const { data: cobrancaCompleta } = await supabaseAdmin
            ...
            .eq("id", cobranca.id)
            .single();
        const selecaoSalva = cobrancaCompleta?.selecao_passageiros_pendente;
        // 6. Ativar assinatura
        ...
        if (selecaoSalva?.passageiroIds && selecaoSalva.passageiroIds.length > 0) {
            ...
            try {
                const resultado = await passageiroService.confirmarSelecaoPassageiros(cobranca.usuario_id, selecaoSalva.passageiroIds, selecaoSalva.franquia || 0);
                logger.info({
                    ...logContext,
            ...
            const { precisaSelecaoManual, assinaturaPendente } = await verificarSelecaoManualNecessaria(cobranca, logContext);
 ...
 */
async function calcularVigenciaFimEAnchorDate(cobranca, dataPagamentoStr, logContext) {
    ...
    let vigenciaFimAtual = assinaturaAtual?.vigencia_fim || null;
    if (!vigenciaFimAtual) {
 ...
 */
async function cancelarCobrancasConflitantes(cobranca, logContext) {
    const billingType = cobranca.billing_type || "subscription";
    if (billingType === "upgrade") {
 ...
 */
async function desativarOutrasAssinaturas(cobranca, logContext) {
 ...
 */
async function verificarSelecaoManualNecessaria(cobranca, logContext) {
 ...
 */
async function ativarAssinatura(cobranca, vigenciaFim, novoAnchorDate, logContext) {
    ...
    logger.info({ ...logContext, vigenciaFim: vigenciaFim.toISOString().split("T")[0] }, "Assinatura do usuário ativada e vigência atualizada");
}
/**
 * Ativa o usuário
 */
async function ativarUsuario(cobranca, logContext) {
 ...
 */
async function ativarPassageirosAutomaticamente(cobranca, assinaturaPendente, logContext) {
    const plano = assinaturaPendente.planos;
    const slugBase = plano.parent?.slug ?? plano.slug;
    logger.info({
        ...logContext,
