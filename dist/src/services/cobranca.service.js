...
export const cobrancaService = {
    async createCobranca(data) {
        if (!data.passageiro_id || !data.usuario_id)
    ...
    async updateCobranca(id, data, cobrancaOriginal) {
        if (!id)
        ...
        if (data.valor !== undefined)
            cobrancaData.valor = data.valor;
        if (data.data_vencimento !== undefined)
            cobrancaData.data_vencimento = data.data_vencimento;
        if (data.status !== undefined)
            cobrancaData.status = data.status;
        if (data.pagamento_manual !== undefined)
            cobrancaData.pagamento_manual = data.pagamento_manual;
        if (data.tipo_pagamento !== undefined)
            cobrancaData.tipo_pagamento = data.tipo_pagamento;
    ...
    async listCobrancasByPassageiro(passageiroId, ano) {
    ...
    async listCobrancasWithFilters(filtros) {
        ...
        if (filtros.passageiroId)
            query = query.eq("passageiro_id", filtros.passageiroId);
    ...
    async countByPassageiro(passageiroId) {
    ...
    async listAvailableYearsByPassageiro(passageiroId) {
        if (!passageiroId)
    ...
    async toggleNotificacoes(cobrancaId, novoStatus) {
