...
export const veiculoService = {
    ...
    async updateVeiculo(id, data) {
        ...
        const veiculoData = { ...data };
        if (data.placa)
            veiculoData.placa = cleanString(data.placa).toUpperCase();
        if (data.marca)
            veiculoData.marca = cleanString(data.marca);
        if (data.modelo)
            veiculoData.modelo = cleanString(data.modelo);
    ...
    async listVeiculos(usuarioId, filtros) {
        ...
        if (filtros?.search) {
            query = query.or(`placa.ilike.%${filtros.search}%,marca.ilike.%${filtros.search}%,modelo.ilike.%${filtros.search}%`);
        }
        if (filtros?.placa)
        ...
        if (filtros?.modelo)
            query = query.eq("modelo", filtros.modelo);
    ...
    async listVeiculosComContagemAtivos(usuarioId) {
        if (!usuarioId)
            throw new Error("Usuário obrigatório");
        const { data, error } = await supabaseAdmin
            .from("veiculos")
    ...
    async toggleAtivo(veiculoId, novoStatus) {
    ...
    async countListVeiculosByUsuario(usuarioId) {
        const { count, error } = await supabaseAdmin
            .from("veiculos")
            .select("id", { count: "exact", head: true })
            .eq("usuario_id", usuarioId);
        if (error)
            throw new Error(error.message || "Erro ao contar veículos");
        return count || 0;
    },
};
