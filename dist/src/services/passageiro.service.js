...
const _preparePassageiroData = (data, usuarioId, ativoDefault = true) => {
...
const _createCobrancaMesAtual = async (passageiroId, passageiroData, usuarioId) => {
...
export const passageiroService = {
    async createPassageiro(data) {
        ...
        try {
            ...
            if (emitir_cobranca_mes_atual) {
                payment = await _createCobrancaMesAtual(newPassageiro.id, passageiroData, data.usuario_id);
            }
    ...
    async finalizePreCadastro(prePassageiroId, data, usuarioId, emitir_cobranca_mes_atual) {
        if (!prePassageiroId)
            throw new Error("ID do pré-passageiro é obrigatório");
        if (!usuarioId)
            throw new Error("Usuário obrigatório");
        ...
        try {
            ...
            const { data: inserted, error: insertPassageiroError } = await supabaseAdmin
                ...
                .select()
                .single();
            if (insertPassageiroError)
                throw insertPassageiroError;
            newPassageiro = inserted;
            // Criar cobrança do mês atual se necessário
            if (emitir_cobranca_mes_atual) {
                payment = await _createCobrancaMesAtual(newPassageiro.id, passageiroData, usuarioId);
            }
    ...
    async updatePassageiro(id, data) {
        ...
        const passageiroData = { ...data };
        if (data.nome)
            passageiroData.nome = cleanString(data.nome, true);
        if (data.nome_responsavel)
            passageiroData.nome_responsavel = cleanString(data.nome_responsavel, true);
        if (data.email_responsavel)
            passageiroData.email_responsavel = cleanString(data.email_responsavel);
        if (data.logradouro)
            passageiroData.logradouro = cleanString(data.logradouro, true);
        if (data.bairro)
            passageiroData.bairro = cleanString(data.bairro, true);
        if (data.cidade)
            passageiroData.cidade = cleanString(data.cidade, true);
        if (data.referencia)
            passageiroData.referencia = cleanString(data.referencia, true);
        if (data.observacoes)
            passageiroData.observacoes = cleanString(data.observacoes, true);
        if (data.valor_cobranca !== undefined)
            passageiroData.valor_cobranca = moneyToNumber(data.valor_cobranca);
        if (data.dia_vencimento !== undefined)
            passageiroData.dia_vencimento = data.dia_vencimento;
        if (data.cpf_responsavel)
            passageiroData.cpf_responsavel = onlyDigits(data.cpf_responsavel);
        if (data.telefone_responsavel)
            passageiroData.telefone_responsavel = onlyDigits(data.telefone_responsavel);
        ...
        if (data.enviar_cobranca_automatica !== undefined) {
            ...
            if (data.enviar_cobranca_automatica === true) {
                ...
                const plano = assinaturas.planos;
    ...
    async listPassageiros(usuarioId, filtros) {
    ...
    `)
            ...
            .order("nome");
        if (filtros?.search) {
            query = query.or(`nome.ilike.%${filtros.search}%,nome_responsavel.ilike.%${filtros.search}%`);
        }
        if (filtros?.escola) {
    ...
    async toggleAtivo(passageiroId, novoStatus) {
    ...
    async getNumeroCobrancas(passageiroId) {
        if (!passageiroId)
            throw new Error("ID do passageiro é obrigatório");
        ...
        if (error)
            throw new Error(error.message || "Erro ao contar cobranças");
        return count || 0;
    },
    async countListPassageirosByUsuario(usuarioId, filtros) {
        let query = supabaseAdmin
            .from("passageiros")
            .select("id", { count: "exact", head: true })
            .eq("usuario_id", usuarioId);
        ...
        if (error)
            throw new Error(error.message || "Erro ao contar passageiros");
        return count || 0;
    },
    async calcularPassageirosDisponiveis(usuarioId, franquiaNova) {
    ...
    async ativarPassageirosAutomaticamente(usuarioId, franquia) {
    ...
    async listarPassageirosParaSelecao(usuarioId, tipo, franquia) {
        if (tipo === "upgrade") {
    ...
    async confirmarSelecaoPassageiros(usuarioId, passageiroIds, franquia) {
        // Validações prévias
