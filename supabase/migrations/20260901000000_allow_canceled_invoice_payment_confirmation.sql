...
-- Atualizado para permitir que faturas que haviam sido marcadas como CANCELED ou FAILED sejam reativadas para PAID
-- caso o pagamento real seja confirmado via Gateway, cancelando as demais faturas pendentes do usuario.
...
BEGIN
    ...
    -- Se ja estiver paga, idempotencia garantida
    ...
    -- 3. Buscar plano associado a fatura ou a assinatura
    ...
    IF v_assinatura.data_vencimento IS NOT NULL AND (v_assinatura.status = 'ACTIVE' OR v_assinatura.status = 'PAST_DUE') THEN
        ...
        -- Se o vencimento base ja passou ha mais de 30 dias, usa agora como base
    ...
    -- 5. Atualizar status da fatura paga para PAID
    ...
    -- 6. Cancelar quaisquer outras faturas pendentes ou com falha do mesmo usuario
    UPDATE public.assinatura_faturas
    SET 
        status = 'CANCELED',
        updated_at = v_now
    ...
    -- 7. Atualizar status da assinatura para ACTIVE e novo vencimento
    ...
    -- 8. Obter dados do usuario para o retorno
