...
BEGIN
  ...
  assinaturas_agg AS (
    SELECT 
      ...
      COUNT(*) FILTER (WHERE status = 'ACTIVE' AND data_vencimento IS NOT NULL)::INT AS active,
      COUNT(*) FILTER (WHERE status = 'ACTIVE' AND data_vencimento IS NULL)::INT AS vitalicio,
...
CREATE OR REPLACE FUNCTION get_admin_user_kpis(p_user_id UUID)
RETURNS JSONB
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT jsonb_build_object(
    'passageirosCount', (SELECT COUNT(*)::INT FROM passageiros WHERE usuario_id = p_user_id),
    'veiculosCount', (SELECT COUNT(*)::INT FROM veiculos WHERE usuario_id = p_user_id),
    'escolasCount', (SELECT COUNT(*)::INT FROM escolas WHERE usuario_id = p_user_id),
    'solicitacoesPendentesCount', (SELECT COUNT(*)::INT FROM pre_passageiros WHERE usuario_id = p_user_id),
    'contratosCount', (SELECT COUNT(*)::INT FROM contratos WHERE usuario_id = p_user_id),
    'contratosAssinadosCount', (SELECT COUNT(*)::INT FROM contratos WHERE usuario_id = p_user_id AND status = 'ASSINADO'),
    'contratosPendentesCount', (SELECT COUNT(*)::INT FROM contratos WHERE usuario_id = p_user_id AND status = 'PENDENTE'),
    'valorTotalContratos', (SELECT COALESCE(SUM(valor_total), 0) FROM contratos WHERE usuario_id = p_user_id AND status = 'ASSINADO')
  );
$$;
