DROP FUNCTION IF EXISTS get_motoristas_latest_activity(TEXT, TEXT, INT, INT, TEXT, TEXT);
...
BEGIN
  ...
  WHERE u.tipo = 'motorista'
    AND (
      ...
      OR (p_subscription_status = 'active_trial' AND (sub.status = 'TRIAL' OR (sub.status = 'ACTIVE' AND sub.data_vencimento IS NOT NULL)))
      OR (p_subscription_status = 'ACTIVE' AND sub.status = 'ACTIVE' AND sub.data_vencimento IS NOT NULL)
      OR (p_subscription_status = 'VITALICIO' AND sub.status = 'ACTIVE' AND sub.data_vencimento IS NULL)
      OR (p_subscription_status NOT IN ('all', 'active_trial', 'ACTIVE', 'VITALICIO') AND sub.status = p_subscription_status)
    )
    AND (
      p_health_status = 'all'
...
DROP FUNCTION IF EXISTS get_motoristas_radar_stats(TEXT);
